...
public:
    void add() {
        ...
        this->counter++;
        if (debug) std::cout << "wg::add: counter=" << this->counter << std::endl;
    }

    void done() {
        std::lock_guard<std::mutex> lock(this->mutex);
        this->counter--;
        if (debug) std::cout << "wg::done: counter=" << this->counter << std::endl;
        this->condition.notify_all();
        if (debug) std::cout << "wg::done: notify_all called\n";
    }

    void wait() {
        std::unique_lock<std::mutex> lock(this->mutex);
        if (debug) std::cout << "wg::wait: calling condition.wait\n";
        this->condition.wait(lock, [this] () { return this->counter == 0; });
        if (debug) std::cout << "wg::wait: condition.wait finished\n";
    }
};
...
void usage()
{
    if (debug) std::cout << "usage: hget [options] (<urls>...)\n";
    if (debug) std::cout << "options:\n"
              << "  -?, -h:      show this help\n"
...
int main(int argc, char **argv)
{
    ...
    if (!wait) {
        if (debug) std::cout << "waiting for all threads\n";
        wg.wait();
        if (debug) std::cout << "all thread have finished\n";
    }
